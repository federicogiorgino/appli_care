...
function EditJobApplicationForm() {
  ...
  const { closeDialog, jobApplication } = useEditJobApplicationDialog()
  const jobApplicationId = jobApplication ? jobApplication.id : ''
  ...
  const form = useForm<EditJobApplicationValues>({
    resolver: zodResolver(editJobApplicationSchema),
    defaultValues: jobApplication as EditJobApplicationValues,
  })
  const { trigger } = form

  const nextStep = async () => {
    const fields = steps[currentStep].fields as Array<
      keyof EditJobApplicationValues
    >
  ...
  const mutation = useMutation({
    mutationFn: (data: EditJobApplicationValues) =>
      updateJobApplication(data, jobApplicationId),
    onSuccess: (data) => {
      if (data.status === 'success') {
        ...
        toast.success('Job application has been updated.')
        closeDialog()
        form.reset()
      } else {
  ...
  const onSubmit = async (data: EditJobApplicationValues) => {
